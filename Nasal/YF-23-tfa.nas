...
var hi_elev_markers = func(lat, lon) {
  ...
  if(marker_status.getValue()) {
    if(lat > -180) {
      htgt = geo.put_model("Aircraft/YF-23/Models/Elevation-marker.ac", lat, lon);
    }
  }
}
#--------------------------------------------------------------------
var dialog = gui.Dialog.new("/sim/gui/dialogs/YF-23/TFA-popup/dialog",
             "Aircraft/YF-23/Dialogs/TFA-popup.xml");
#--------------------------------------------------------------------
